...
def test_une_piste_est_immuable() -> None:
    """Une piste modifiée en cours de file rendrait la fenêtre de non-répétition
    incohérente avec ce qui a été joué."""
    p = track("id1", "Bowie")
...
def test_le_genre_peut_manquer() -> None:
    """Une bibliothèque réelle a des morceaux sans genre ; les refuser amputerait
    la radio."""
...
def test_une_duree_nulle_ou_negative_est_refusee(secondes: int) -> None:
    """Les jingles se programment sur les durées : une durée fausse ferait
    glisser la grille sans rien signaler."""
...
def test_une_piste_sans_annee_reste_valable() -> None:
    """Une partie de la bibliothèque n'a pas d'année (docs/subsonic.md §4.1) : la
    piste reste valable, elle ne participe pas aux époques."""
