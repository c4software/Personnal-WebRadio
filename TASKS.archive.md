...
## GOAL-080 — Ce qu'une plage podcasts expose, et que la revue a trouvé

Ouvert le 2026-09-06 par la revue de GOAL-077, avant tout déploiement.

- [x] **GOAL-080-T01** — Une plage fermée ne fait plus lire ses flux. Le
      contrôle d'ouverture reposait sur `slot_start`, vrai jusqu'à la veille de
      l'occurrence suivante : les six flux de l'auteur étaient lus **du samedi
      20 h au lundi soir**, pour trois heures d'antenne. Une plage déclare sa
      fin, elle se sait fermée sans qu'on lise rien.
- [x] **GOAL-080-T02** — Deux émissions ne déclarent plus le même flux.
      Constaté dans la configuration écrite la veille : LEGEND y était déclarée
      seule le mardi **et** dans la plage du week-end. Chacune tient sa propre
      mémoire, donc le même épisode serait passé deux fois. La déclaration du
      mardi a été retirée — l'auteur avait demandé LEGEND dans la plage.
      La règle de recouvrement voit aussi, maintenant, une plage qui enjambe
      minuit.
- [x] **GOAL-080-T03** — Documentation et commentaires remis d'aplomb : SPECS
      §6 (les clés `feeds`, `end`, `cache_seconds` y manquaient),
      ARCHITECTURE §5 (la clé de mémoire et ce qu'un flux ajouté ou retiré
      change), trois commentaires qui décrivaient un mécanisme que le code
      n'avait plus, et une mesure recopiée du relevé au lieu d'y renvoyer.
      Le relevé lui-même dit désormais que « les six » est une **somme**, pas
      une lecture d'affilée, et que ses coûts varient d'un essai à l'autre.
- [x] **GOAL-080-T04** — Tests : la frontière exacte entre deux plages qui se
...
- [x] **GOAL-080-T05** — **Le délai d'attente des flux pouvait faire couper
      ...
      échec. Un hébergeur qui absorbe les paquets — ni refus, ni réponse —
...
**Clos le 2026-09-06.** Cinq tâches, deux commits. Rien de tout cela n'avait
été entendu : la revue l'a trouvé en lisant, avant tout déploiement.

**Reste à écouter** (AGENTS.md §4.1) : le début d'une plage, pour vérifier
...
- [x] **GOAL-086-T06** — La documentation transversale, et la clôture. Le README
      dit ce que « Passer » fait d'un épisode de plage, ce que les deux boutons
      valent sur quoi, et ce qu'un redémarrage de `radio` donne à voir — une
      antenne `inconnu` et des votes refusés le temps d'un battement. SPECS.md
      §4.6 n'annonce plus les deux commandes comme disponibles « sauf pendant un
      jingle ou un flash », alors que le tableau qui suit refuse aussi les
      émissions ; §5.1 gagne la ligne du redémarrage de `radio` ; la clé
      `liquidsoap.url` nomme les quatre ordres, `/skip-fresh` et `/announce`
      compris. ARCHITECTURE.md les nomme aux trois mêmes endroits — le schéma du
      §4, la table de confinement, les ordres hors verrou — et compte
      désormais trois routes entrantes, pas deux.
      **Ce que la relecture a établi**, et qui n'est corrigé nulle part ici :
      les deux entrées tirées par `/skip-fresh` ne sont **pas** deux épisodes.
      `Shows.due()` rend `None` tant qu'un épisode demandé n'a pas pris
      l'antenne, donc la seconde est une musique ; et si c'est elle qui démarre
      la première, `_oublier_les_demandes_anterieures` oublie l'épisode du
      registre et `_signaler_l_emission` appelle `Shows.dropped()`. L'épisode
      joue ensuite quand même — `_restaurer` le déclare d'après ses annotations,
      l'antenne est juste — mais **rien ne l'inscrit comme diffusé** : il reste
      repiochable, et peut repasser dans la même plage. Rejoué sur la pile
      réelle avec `FrozenClock`, hors dépôt ; consigné en résidu à écouter dans
      docs/liquidsoap.md §14, avec le point incertain qu'il ouvre sur
      l'hypothèse de rang de GOAL-083.

**GOAL-086 est clos le 2026-09-06.** Deux défauts, cinq corrections, trois
relevés. Le redémarrage aveugle d'abord : un processus `radio` redéployé en
plein épisode se croyait sur de la musique et acceptait un « Passer » ; il dit
maintenant `inconnu` et refuse les votes, puis fait redire au diffuseur ce qu'il
joue et lui fait redemander son avance — l'ignorance dure un battement, et
chaque entrée porte sa nature dans son préfixe `annotate:`. L'avance ensuite :
la clé qui la date connaît les cases de podcasts et l'épisode demandé sans avoir
commencé, donc la musique tirée avant l'ouverture d'une plage ne prend plus
l'antenne entre deux épisodes. Et « Passer » pioche désormais un autre épisode
pendant une plage : `/skip-fresh` remplace l'avance du diffuseur avant de
sauter, le refus ne subsiste que sans épisode neuf, et « Encore » reste refusé
sur toute émission. Trois relevés sur maquette (docs/liquidsoap.md §12, §13,
§14) ont précédé chaque mécanisme, dont un qui a écarté la variante à un seul
tirage : `set_queue` détruit les requêtes qu'on lui repasse.

**Reste à écouter** (AGENTS.md §4.1) :

- un « Passer » pendant une plage de podcasts avec un vrai épisode lourd : le
  blanc ou son absence à la bascule, l'entrée fraîche qui prend l'antenne, et
  l'épisode qui suit — la maquette simulait 8 s de résolution, pas 120 Mo ;
- un redéploiement en pleine plage : l'antenne restaurée par la ré-annonce, les
  votes rouverts au premier battement, et aucune musique entre deux épisodes ;
- le motif de refus quand aucun flux n'a plus d'épisode neuf, tel qu'il
